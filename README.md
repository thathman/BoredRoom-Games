...
| Whot | `whot` | 2–8 | Card game | Nigerian shape-and-number showdown. |
...
Whot should behave like a real card game: full deck, deterministic shuffle, draw/market pile, discard pile, shape/number matching, Whot card call-shape, configurable special cards, round scoring, bot support, table-style public view, and private hand controller view.

### Ludo

Ludo should behave like a real board game: seeded dice, player paths, home yards, safe squares, captures, home stretch, exact finish rules, extra turn on six, optional quick mode, animated public board, and private dice/token controller.

### Connect 4

Connect 4 should support a 6x7 board, legal column drops, horizontal/vertical/diagonal wins, draw detection, solo/team mode, best-of rounds, animated public board, and controller column controls.

### Bible Timeline Rush

Bible Timeline Rush should support hidden canonical timelines, randomized visible order, drag/drop controller ordering, all-submit reveal, countdown-to-next-round, difficulty levels, Bible reference explanations, and no repeated prompts.

### Color Wahala

Color Wahala should be a true Stroop/reaction game with displayed word and ink colour separated, misleading prompt logic, safe flag-colour prompts, answer timer, speed scoring, reveal explanations, and no repeated prompts.

### Endless Tic Tac Toe

Endless Tic Tac Toe should be rolling tic-tac-toe with limited active marks, oldest mark removal, move history, team mode, and final board review.

### Half & Half

Half & Half should be a social midpoint/split game with split-vote mode, midpoint guess mode, distribution reveal, median/split scoring, and clear recap.

### Faith Feud

Faith Feud should be a Family Feud-style game with survey setup/generation, team mode, answer aliases, fuzzy matching, top answers, strikes, steals, reveal board, and team scoring.

### Hustle

Hustle should be a Snakes-and-Ladders-style Nigerian hustle board with dice, tokens, ladders/opportunities, snakes/wahala, event cards, animated board, and quick/normal mode.

### Oga Landlord

Oga Landlord should be a Monopoly-inspired Nigerian property game with roll/move, buy/pass, rent, ownership, chance/wahala cards, bankruptcy/end conditions, quick/normal mode, public board, and private portfolio controls.

### Logo Guesser

Logo Guesser should support a logo bank, obscured logo stages, blur/crop/pixelate/mask reveal, typed or multiple-choice answers, hints, no repeated logos, and safe configured logo sources only.

### Word Wahala

Word Wahala should be Scrabble-like with tile bag, racks, board, letter values, placement validation, cross-word validation, dictionary modes, pass/swap, and score preview.

### Market Price

Market Price should use curated data and optional cached Supermart.ng snapshots. It must never fetch Supermart during active gameplay. Product detail pages are the source of truth, and reveal screens should show product images, cached price timestamps, and source credit.
...
Pidgin Translator should be voice-first but privacy-safe. The default mode is fastest-correct speed voice with text fallback. It must not continuously listen, stream room audio, broadcast raw audio, or let transcription completion time decide who was fastest.

### Who Sabi Pass?

Who Sabi Pass should support large question banks, categories, difficulty, timers, reveal explanations, no repeated questions, and optional AI-generated questions with moderation and fallback.
...
The build script reads manifests, stages game source, writes release metadata, packages `.tgz` artifacts, calculates SHA-256, signs artifact digests, and writes `catalog.json`.
...
The test command runs dictionary sync where needed, runtime contract tests, and per-game tests under `games/*/tests/*.test.mjs`.

Every game should have tests for runtime contract, legal intents, illegal intent rejection, settings validation, scoring, timers, snapshot/restore, finish/recap, projections, bots where supported, teams where supported, no-repeat content, and external-data fallback where applicable.
...
3. Do not bundle third-party images or product data unless permission allows it.
...
See [`CONTRIBUTING_GUIDE.md`](./CONTRIBUTING_GUIDE.md).
...
This repository is licensed under the MIT License notice in [`LICENSE.md`](./LICENSE.md).
