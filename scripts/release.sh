...
RELEASE_TAG="${RELEASE_TAG:-v1.7.0.1}"
export RELEASE_TAG
...
echo "==> 4/4 Changed files (dist is intentionally ignored; catalog is tracked):"
git status --short dist catalog.json
echo
echo "Next:"
echo "  git add catalog.json"
echo "  git commit -m \"Release ${RELEASE_TAG}: rebuilt runtimes\""
echo "  git tag ${RELEASE_TAG} && git push --tags"
echo "  gh release create ${RELEASE_TAG} dist/*.tgz catalog.json --title \"${RELEASE_TAG}\" --notes \"BoredRoom-Games ${RELEASE_TAG}\""
echo
