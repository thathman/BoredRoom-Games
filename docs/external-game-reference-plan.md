...
- **Whot ← mykeels/whot (MIT):** Re-audited for `v1.6.0.0`. BoredRoom uses the same 54-card deck and special-move map, including the reference’s Star 8 two-player suspension and General Market play-again behavior as defaults. House variants remain explicit settings. The unified runtime adds best-of-five scoring, automatic card-count calls, snapshots, deterministic bots, timers, and role projections. Attribution is included in the runtime and signed artifact.
